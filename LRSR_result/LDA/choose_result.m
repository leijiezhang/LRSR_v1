...
for k = cut_mark+1:car_num
    ...
    for i= 1:7
        for j= 1:7
            ...
            real_error=real_error_arr(k,1);
            real_k=real_error_arr(k,2);
            if(real_error<least_error&&real_error>0&&real_k>bestK_before)
                LRSR_arr(1,k)=real_error;
                LRSR_arr(2,k)=real_k;
                least_error=real_error;
                bestK=real_k;
...
% save USPS_LPP_final.mat LRSR_result USSR_error LRSR_error car_idx best_k result_com USSR_acc LRSR_acc;
% clear;
