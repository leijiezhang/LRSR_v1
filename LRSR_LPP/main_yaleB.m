...
%                   clear eigvector;
                ...
                temp_idx=floor(linspace(1,car_idx(i),20));
                size_temp=size(temp_idx,2);
                for k=1:size_temp
                    [U,V]=objFun(options,Y,fea_Train,temp_idx(k));
%                   clear eigvector;
                    fea_Test_new = fea_Test*U*V;
                    fea_Train_new = fea_Train*U*V;
                    clear U V;
                    ...
                    clear label;
                    printStr=strcat('dimension:',num2str(i),'---inter medium rank:',num2str...
                        (temp_idx(k)),'==>error rate:',num2str(Error1));
                    disp(printStr);
                    if Error1<error_k_least
                        error_k_least=Error1;
                        k_least=temp_idx(k);     
...
save -v7.3 YaleB_LPP_67.mat LRSR_result USSR_error;
toc;